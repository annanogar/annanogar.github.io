 ...
 *
 * This file is the main entry point for Sonic, and it sets up the runtime environment, parses command-line arguments, and runs the appropriate tasks based on the provided arguments.
 * The actual tasks that can be run are defined in the "tasks.js" file, and this index file is responsible for executing those tasks based on the command-line arguments.
 * The runtime environment variables are set based on the provided flags and tasks, and can be used by the tasks to determine how to run (e.g. in production mode, with colors, with cache, etc.).
 *
 * Each task may have its own dependencies, which are listed in the comments at the top of each action file (sonic/actions/*.js).
 * These dependencies are not required to run Sonic itself, but they are required to run the specific tasks that use them.
 *
 * Tasks are executed sequentially, in the order provided, and can be easily added, changed, or removed.
 */
