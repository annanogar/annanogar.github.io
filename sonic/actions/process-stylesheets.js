 ...
 * This is used to process source stylesheets in place, and to ensure that they are processed before being compiled to the destination directory.
 *
 * Usage:
 *   processStylesheets(sourceGlobs, destinationPath)
 * Arguments:
 *   - sourceGlobs (string | string[]) - The glob pattern(s) for the source files
 *   - destinationPath (string) - Misnomer: this receives the source root and is only used to relativize
 *     sourcemap "sources". PostCSS output is written back over each matched file IN PLACE, not to a separate dir.
 *
...
const processSource = async (path = '', destinationPath = '', compiler = null, hashCache = null) => {
  ...
  // Write the processed CSS back over the source file (in place); resolvedPath is the source path
