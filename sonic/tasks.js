...
import actionConvertSourceImages from './actions/convert-source-images.js'
...
import actionValidateAlts from './actions/validate-alts.js'
...
export const tasks = {
  ...
  convertSourceImages: async (paths = config.sourceImages.sourceGlobs) => await actionConvertSourceImages(paths, config.sourceImages.sourcePath, config.sourceImages.destinationPath, config.sourceImages.deleteOriginals, config.sourceImages.ratios),
...
export const composedTasks = {
  ...
  convert: async () => {
    if (runtime.logLevel !== 'quiet') {
      process.stdout.write(`  ${runtime.colors.accent}Converting${runtime.colors.reset} source images...\n`)
    }

    await tasks.convertSourceImages()
  },
